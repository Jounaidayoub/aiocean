...
const providers = {
  ...
  google: {
    createUrl: (q: string) =>
      `https://google.ai?${new URLSearchParams({
        q,
      })}`,
    icon: (
      <svg
      ...
      >
        <title>Google</title>
        <path d="M12.48 10.92v3.28h7.84c-.24 1.84-2 5.36-7.84 5.36-5.08 0-9.24-4.2-9.24-9.4s4.16-9.4 9.24-9.4c2.92 0 4.88 1.24 6 2.32l2.6-2.6C19.64 1.84 16.32 0 12.48 0 5.56 0 0 5.56 0 12.48S5.56 24.96 12.48 24.96c7.24 0 12.08-5.08 12.08-12.28 0-.84-.08-1.48-.2-1.76h-11.88z" />
      </svg>
    ),
...
export type OpenInGoogleProps = ComponentProps<typeof DropdownMenuItem>;

export const OpenInGoogle = (props: OpenInGoogleProps) => {
  const { query } = useOpenInContext();
  return (
    <DropdownMenuItem {...props} render={<a className="flex items-center gap-2" href={providers.google.createUrl(query)} rel="noopener" target="_blank" />}><span className="shrink-0">{providers.google.icon}</span><span className="flex-1">{providers.google.title}</span><ExternalLinkIcon className="size-4 shrink-0" /></DropdownMenuItem>
  );
};
